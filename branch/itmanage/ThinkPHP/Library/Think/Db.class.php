...
class Db {
     ...
     */
    static public function getInstance($config=array(), $tableName = '') {
        //2019-05-14 白景琦修改增加判断 代码“ strpos($tableName, '||') !== false”，为了支持同一数据库链接，区别普通oracle和带有clob字段的数据库连接
        if(strpos($tableName, '||') !== false){
            $md5    =   md5(serialize($config). $tableName);
        }else{
        ...
        if(!isset(self::$instance[$md5])) {
            ...
            //2018-11-15 白景琦新增，以 || 分隔，可在表名后增加 || + 字符串，目的是为了扩展oracle 特殊字段类型clob，如果以后需要继续扩充，可参照该方法
            if(!empty($tableName) && strpos($tableName, '||') !== false) {
                $ext = ucfirst(explode('||', $tableName)[1]);
                $options['type'] .= $ext;
            }
    ...
    static private function parseConfig($config){
        if(!empty($config)){
            ...
            $dbc=C(session("satellite"));
            $config = array (
                'type'          =>  $dbc['DB_TYPE'],
                'username'      => $dbc['DB_USER'],
                'password'      => $dbc['DB_PWD'],
                'hostname'      =>  $dbc['DB_HOST'],
                'hostport'      =>  $dbc['DB_PORT'],
                'database'      =>  $dbc['DB_NAME'],
                'dsn'           =>  C('DB_DSN'),
                'params'        =>  C('DB_PARAMS'),
                'charset'       =>  $dbc['DB_CHARSET'],
