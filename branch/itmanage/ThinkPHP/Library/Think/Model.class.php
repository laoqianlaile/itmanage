...
class Model {
    ...
    public function __construct($name='',$tablePrefix='',$connection='') {
        ...
        if(is_null($tablePrefix)) {// 前缀为Null表示没有前缀
            ...
            $this->tablePrefix = $dbc['DB_PREFIX'].session("jieduancode");
        }
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            ...
            if(C('DB_FIELDS_CACHE')) {
                ...
                $fields = F('_fields/'.session("satellite")."/".strtolower($db.'.'.$this->tablePrefix.$this->name));
                if($fields) {
                    $this->fields   =   $fields;
                    if(!empty($fields['_pk'])){
                        $this->pk       =   $fields['_pk'];
    ...
    public function flush() {
        ...
        if(C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?:C('DB_NAME');
            F('_fields/'.session("satellite")."/".strtolower($db.'.'.$this->tablePrefix.$this->name),$this->fields);
        }
    }
    ...
    public function db($linkNum='',$config='',$force=false) {
        ...
        if(!isset($this->_db[$linkNum]) || $force ) {
            ...
            //2018-11-15 白景琦加入$this->name 参数，以识别oracle的clob特殊字段
            $this->_db[$linkNum]            =    Db::getInstance($config, $this->name);
            if(strpos($this->name, '||') !== false) {
                $this->name = ucfirst(explode('||', $this->name)[0]);
            }
