...
use Think\Model;
class SwitchnewinfoModel extends BaseModel {
    ...
    function gtOneMacInfoData($macaddress,$queryparam){
        $Model = M();
        $sql_select="
                select * from it_switchnewinfo s where s.sw_macaddress='".$macaddress."'";
        $sql_count="
                select count(*) c from it_switchnewinfo s where s.sw_macaddress='".$macaddress."'";
...
//        echo $sql_count;die;
    ...
    function SWPortDownData($queryparam){
        ...
        if ("" != $queryparam['sort']) {
            ...
            $result = M('switchnewinfo')->where($map)->where('sw_macaddress is not null')->order($order)->limit($limit,$offset)->select();
        }else{
            $result = M('switchnewinfo')->where($map)->where('sw_macaddress is not null')->order("sw_atpid desc")->limit($limit,$offset)->select();
        }
//        echo M('switchnewinfo')->_sql();
        $count  = M('switchnewinfo')->field('count(*) c')->where($map)->where('sw_macaddress is not null')->select();
    ...
    function SWPortUpData($queryparam){
        ...
        if ("" != $queryparam['sort']) {
            $order = array($queryparam['sort']=>$queryparam['sortOrder']);
            $result = M('switchnewinfo')->where($map)->where('sw_macaddress is null')->order($order)->limit($limit,$offset)->select();
        }else{
            $result = M('switchnewinfo')->where($map)->where('sw_macaddress is null')->order("sw_atpid desc")->limit($limit,$offset)->select();
        }
//        echo M('switchnewinfo')->_sql();die;
        $count  = M('switchnewinfo')->field('count(*) c')->where($map)->where('sw_macaddress is null')->select();
    ...
    public function getSWInfoByIP($switchip){
        ...
        $portInfo = M('switchnewinfo')->where($where)->field('sw_ipaddress,sw_interface,sw_vlan')->select();
        return $portInfo;
    }
}