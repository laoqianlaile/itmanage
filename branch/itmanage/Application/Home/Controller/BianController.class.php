...
class BianController extends BaseController
{
    ...
    public function inAll(){
        $ids = I('post.bian');
        $id = explode(',',$ids);
        if(in_array('1',$id) && in_array('3',$id)){
            exit(makeStandResult(-1, '变更责任人与变更部门处室不能同时勾选！'));
        }else if(in_array('4',$id) && in_array('5',$id)){
            exit(makeStandResult(-2, '变更主板网卡与变更终端设备不能同时勾选！'));
        }else if(empty($ids)){
            exit(makeStandResult(-3, '变更类型必须勾选'));
        }else{
